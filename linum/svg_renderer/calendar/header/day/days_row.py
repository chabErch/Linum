...
from linum.svg_renderer.calendar.header.day.day_off_day_cell import DayOffDayCell
from linum.svg_renderer.calendar.header.day.workday_day_cell import WorkdayDayCell
...
class DaysRow(Row):
    ...
    def __init__(self, start: date, length: int, width: float,
        ...
        self.workday_style = workday_style or Style('default day')
        self.day_off_style = day_off_style or Style('default day-off day')
    ...
    def cells(self) -> List[DayCell]:
        ...
        for i in range(self.length):
            ...
            if is_day_off(d, self.days_off, self.workdays):
                cell = DayOffDayCell(d, cell_width, self.day_off_style)
            else:
                cell = WorkdayDayCell(d, cell_width, self.workday_style)
            cells.append(cell)
        return cells
